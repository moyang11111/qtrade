...
class PaperTradingEngine:
    ...
    def trade_at_quote(self, account_id: str, symbol: str, side: str, qty: int, note: str = "") -> Dict:
        ...
        if req.side == "buy" and quote.limit_up is not None and float(quote.price) >= float(quote.limit_up):
            raise ValueError(f"涨停无法买入 {req.symbol}")
        if req.side == "sell" and quote.limit_down is not None and float(quote.price) <= float(quote.limit_down):
        ...
        with self._lock, self._connect() as conn:
            ...
            if not account:
                raise ValueError(f"account {req.account_id} not found")
            if req.side == "buy":
                amount = req.qty * float(quote.price)
                commission = calc_commission(amount, req.symbol)
                need = amount + commission
                available = float(account["cash"]) - float(account["frozen_cash"])
            ...
            conn.execute(
                """
                INSERT INTO orders(order_id, account_id, symbol, side, order_type, limit_price, qty, reserved_cash, filled_qty, avg_fill_price, status, note, last_checked_at, validity, created_at, updated_at)
                VALUES(?, ?, ?, ?, ?, NULL, ?, ?, 0, NULL, 'open', ?, ?, 'day', ?, ?)
                """,
                (order_id, req.account_id, req.symbol, req.side, req.order_type, req.qty, 0.0, req.note, ts, ts, ts),
            )
            market_date = str(quote.timestamp).split(" ")[0]
